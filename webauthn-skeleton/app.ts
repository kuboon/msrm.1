import { opine, json, serveStatic, OpineSession, HTTPSOptions, HTTPOptions, WrappedRequest, ConnInfo} from "./deps.ts";
...
app.use(json());
app.use(serveStatic("./webauthn-skeleton/public/static"));
...
export async function handler(request: Request, connInfo: ConnInfo){
	...
	app(opineRequest);
	const res = await opineRequest.finalResponse;
	request.session = opineRequest.session;
	return res;
}
